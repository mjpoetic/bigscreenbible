...
- AMP and NIrV chapter requests use one whole-chapter YouVersion HTML passage request with notes disabled. The function splits the provider's verse milestones, strips unsupported markup, and preserves paragraph starts, section headings, poetic line breaks, and source `wj` spans as display metadata. Scripture responses use `Cache-Control: no-store`.
- Red-letter annotations remain translation-specific. The authorized AMP payload currently supplies `wj` spans, while sampled NIrV payloads do not; NIrV therefore remains normal text rather than borrowing another translation's editorial speaker boundaries.
