...
- AMP and NIrV chapter requests use one whole-chapter YouVersion HTML passage request with notes disabled. The function splits the provider's verse milestones, strips provider markup, and preserves source `wj` spans as words-of-Jesus character ranges for the app's red-letter renderer. Scripture responses use `Cache-Control: no-store`.
