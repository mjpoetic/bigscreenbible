...
const focusSettingsSearchTargetSource = extractFunction("focusSettingsSearchTarget");
const settingsTextSizeSource = extractFunction("settingsTextSizeMarkup");
const settingsScriptureFontSource = extractFunction("settingsScriptureFontMarkup");
const settingsDeepSearchResultsSource = extractFunction("settingsDeepSearchResultsMarkup");
const mainSettingsRootSource = extractFunction("mainSettingsRootMarkup");
const mainSettingsPanelSource = extractFunction("mainSettingsPanelMarkup");
const settingsDestinationRowSource = extractFunction("settingsDestinationRow");
...
assert.match(displaySource, /settingsDisclosure\("display", "Scripture display"/);
...
assert.match(readingSource, /settingsDisclosure\("reading", "Reading & navigation"/);
...
assert.match(applySettingsSearchSource, /data-settings-search-item/);
assert.match(applySettingsSearchSource, /data-settings-search-group/);
assert.match(bindSettingsSearchSource, /addEventListener\("input"/);
assert.match(applySettingsSearchSource, /data-settings-search-only/);
assert.match(applySettingsSearchSource, /data-settings-browse-only/);
assert.match(focusSettingsSearchTargetSource, /scrollIntoView/);
assert.match(focusSettingsSearchTargetSource, /settings-search-target-focus/);
...
assert.match(mainSettingsRootSource, /settingsSearchMarkup\(prefix\)[\s\S]*?Bible version[\s\S]*?settingsTextSizeMarkup[\s\S]*?settingsScriptureFontMarkup/);
assert.match(mainSettingsRootSource, /settingsDeepSearchResultsMarkup\(prefix\)/);
assert.match(settingsTextSizeSource, /settings-quick-text-actions/);
assert.match(settingsTextSizeSource, /FullscreenButton/);
assert.match(settingsScriptureFontSource, /Scripture font/);
assert.match(settingsDeepSearchResultsSource, /GameVolume/);
assert.match(settingsDeepSearchResultsSource, /AutoScrollSpeedLabel/);
assert.match(settingsDeepSearchResultsSource, /AppUpdateButton/);
assert.match(mainSettingsRootSource, /Appearance & Text[\s\S]*?Scripture & Reading[\s\S]*?Sounds[\s\S]*?Sharing & Printing[\s\S]*?Startup & Notifications[\s\S]*?App, Help & Legal/);
assert.match(mainSettingsPanelSource, /data-settings-back/);
assert.match(settingsDestinationRowSource, /data-settings-page/);
assert.match(mobileSettingsPanelSource, /mainSettingsPanelMarkup\("mobile"\)/);
assert.match(topbarSource, /mainSettingsPanelMarkup\(\)/);
assert.match(presentationSource, /presentationSettingsPanelMarkup\(version, customFontField\)/);
...
assert.match(mainSettingsPanelSource, /settings-page-enter-\$\{settingsPageTransition\}/);
assert.match(source, /settingsPageTransition = "back"/);
assert.match(styles, /\.settings-destination-row/);
assert.match(styles, /@keyframes settingsDrillForward/);
