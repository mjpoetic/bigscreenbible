...
assert.match(extractFunction("mountMobileGameControls"), /\.reference-rush-hints, :scope > \.trivia-actions/);
...
assert.match(styles, /\.games-options-drawer,[\s\S]*?\.games-controls-drawer \{[\s\S]*?top: auto;[\s\S]*?animation-name: gamesDrawerInBottom;/);
