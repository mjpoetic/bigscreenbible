...
assert.equal(source.match(/id="openTriviaReference"/g)?.length, 7);
...
assert.match(openReference, /game\?\.type === "crossword"[\s\S]*?game\.reference/);
assert.match(openReference, /game\?\.type === "hidden-word"[\s\S]*?game\.rounds\?\.\[game\.index\]\?\.reference/);
